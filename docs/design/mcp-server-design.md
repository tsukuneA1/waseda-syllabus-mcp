...
```
apps/backend/packages/mcp-server/
├── pyproject.toml
└── src/
    └── waseda_mcp/
        ├── __init__.py
...
async def search_syllabus(
    query: str,
    year: int | None = None,
    semester: Literal["spring", "fall", "full"] | None = None,
    limit: int = 10,
) -> list[CourseSummary]:
    """
    ...
    Args:
        ...
        year: 対象年度（省略時は最新年度）
        semester: 開講学期。"spring"=春学期, "fall"=秋学期, "full"=通年
...
class CourseSummary(BaseModel):
    ...
    instructors: list[str] # 担当教員名（複数担当あり）
    semester: str          # 開講学期 ('spring' | 'fall' | 'full' | 'unknown')
