...
│   ├── CLAUDE.md               # このファイル
│   ├── AGENTS.md               # Codex 向けガイド（CLAUDE.md のシンボリックリンク的役割）
│   ├── README.md
│   ├── apps/
...
# worktree 間の素早い移動（wt コマンド）
# セットアップ: source scripts/wt.sh を ~/.bashrc / ~/.zshrc に追記
wt main      # main worktree へ移動
wt 14        # WMCP-14-... worktree へ移動
wt WMCP-14   # 同上
```
...
- `--head` と `--base main` を明示して `gh pr create` を実行する
